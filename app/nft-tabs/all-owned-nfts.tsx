...
export default function AllOwnedNftsTab({
  searchQuery,
  contractId,
  address,
}: {
  ...
  const {
    ...
    queryFn: async () => {
      return await getWalletNFTs(address as string);
    },
    placeholderData: keepPreviousData,
  });

  useEffect(() => {
    if (ownedNfts && ownedNfts?.result) {
      if (searchQuery && searchQuery != "") {
        console.log("one item is : ", ownedNfts?.result[0]);
        const newResult = ownedNfts?.result.filter((item: any) =>
        ...
        setFilteredCollectionsData(ownedNfts?.result);
      }
    }
  }, [searchQuery, ownedNfts]);
...
import { getWalletNFTs } from "@/http/client";
