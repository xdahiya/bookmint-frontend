...
export default function CreatedNftsTab({
  searchQuery,
  contractId,
  collections,
  address,
  ...
  const [filteredCollectionsData, setFilteredCollectionsData] = useState<any>(
    []
  );

  const queriesData = useQueries({
    queries: (collections || []).map((param: any) => ({
      queryKey: ["collectionnfts", param],
      queryFn: () => getContractNFTs(param as string),
    })),
  ...
  useEffect(() => {
    if (!queriesData.pending) {
      try {
        const filteredMergeData = queriesData.data.filter(
          (data: any) => !!data
        );

        let filteredMergeDataFinal: any = [];

        filteredMergeData.forEach((data: any) => {
          filteredMergeDataFinal = [...filteredMergeDataFinal, ...data.result];
        });

        if (searchQuery && searchQuery != "") {
          const newResult = filteredMergeDataFinal.filter((item: any) =>
          ...
          setFilteredCollectionsData(filteredMergeDataFinal);
        }
      } catch (error) {
        console.log("error is : ", error);
      }
    }
  }, [queriesData, searchQuery]);
  ...
  //     }
  //   }
  // }, [filteredMergeDataFinal, searchQuery]);
  ...
  const onFilterChange = (changedFields: any) => {
    ...
    //     // );
    //     // setFilteredCollectionsData(newResult);
    //     setFilteredCollectionsData(ownedNfts);
    //   } else {
    //     setFilteredCollectionsData(filteredMergeDataFinal);
    //   }
    // }
    ...
    //     // );
    //     // setFilteredCollectionsData(newResult);
    //     setFilteredCollectionsData(listedNfts);
    //   } else {
    //     setFilteredCollectionsData(filteredMergeDataFinal);
    //   }
    // }
...
import { getCollections, getContractNFTs } from "@/http/client";
